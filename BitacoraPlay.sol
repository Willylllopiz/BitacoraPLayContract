...
abstract contract MoneyBox {
    function addToBalance(address _user, uint amount) external virtual payable;
}

contract BitacoraPlayBasic{
    ...
    mapping(address => CareerPlan) public careerPlan;
...
contract BitacoraPlay is BitacoraPlayBasic{
    ...
    constructor(address _externalAddress, address _rootAddress, address _moneyBox) public {
        ...
        careerPlan[_rootAddress].activeCareerPlan = true;
    }
    ...
    function payCareerPlanActivation(address _user) internal {
        careerPlan[_user].activeCareerPlan = true;
        careerPlan[users[_user].referrer].accumulatedDirectPlanCareer ++;

    }

    function updateActivePlanCareer(uint8 _level, address _referrerAddress) private {
        if(_level > 0 && _referrerAddress != rootAddress) {
            careerPlan[_referrerAddress].accumulatedPlanCareer ++;
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        careerPlan[userAddress].activeCareerPlan = false;
    ...
    function updateActiveMembers(uint8 _level, address _referrerAddress) private {
        if(_level > 0 && _referrerAddress != rootAddress){
            ...
            users[_referrerAddress].accumulatedPayments += 0.36e18;
